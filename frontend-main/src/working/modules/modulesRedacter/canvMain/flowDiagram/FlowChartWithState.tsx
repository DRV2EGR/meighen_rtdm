...
export class FlowChartWithState extends React.Component<IFlowChartWithStateProps, IChart> {
  ...
  handleSaveEvent() {
    ...
    console.log("uuid = ", this.props.compUUID);
    let body = {
      "body": JSON.stringify(this.props.workFlowValue())
    }

    const cookies = new Cookies();
    let a = cookies.get('accessToken');

    fetch('/presenter/api/modules/module?uuid='+ this.props.compUUID.compUUID, {
      method: 'PUT',
      headers: {
  ...
  public render () {
    ...
    return (
      <React.Fragment>
        ...
        />
        <SaveBox className="save-btn">
          <SaveBtn onClick={this.handleSaveEvent}>Сохранить</SaveBtn>
        </SaveBox>
      </React.Fragment>
    )
  }
